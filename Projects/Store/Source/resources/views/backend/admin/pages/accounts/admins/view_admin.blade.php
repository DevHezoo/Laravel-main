...
@section('dashboard')
              ...
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Manage Admins /</span> Account</h4>
    ...
    <form method="post" action="{{ route('admin.edit.admin.info',$user->id) }}"  enctype="multipart/form-data" >
      @csrf
...
<img
  src="{{ (!empty($user->photo)) ? asset('/frontend/upload/admin_images/'.$user->photo) : asset('/frontend/upload/no_image.jpg') }}"
  alt="user-avatar"
...
<label for="imageUpload" tabindex="0">
  <span class="d-none d-sm-block">
    ...
    <form method="post" action="{{ route('admin.edit.admin.pass',$user->id) }}" >
      @csrf
...
<script type="text/javascript">
  ...
  $('#imageUpload').change(function () {
    ...
    $('#reset').click(function() {
        ...
        $('#imagePreview').attr('src', "{{ (!empty($user->photo)) ? asset('/frontend/upload/admin_images/'.$user->photo) : asset('/frontend/upload/no_image.jpg') }}");
    });
...
document.getElementById('deactivateButton').addEventListener('click', function() {
    if (confirm("Are you sure you want to delete your account?\nOnce you delete your account, there is no going back. Please be certain.")) {
        ...
        window.location.href = '/admin/delete/admin/{{$user->id}}';
    } else {
