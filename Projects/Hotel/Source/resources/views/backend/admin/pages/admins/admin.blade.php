...
@section('admin')
    ...
    <form method="post" action="{{ route('update.admin.info',$admin->id) }}"  enctype="multipart/form-data" >
      @csrf
...
<img
  src="{{ (!empty($admin->photo)) ? asset('frontend/upload/admin_images/'.$admin->photo) : asset('frontend/upload/no_image.jpg') }}"
  alt="user-avatar"
...
<label for="imageUpload" tabindex="0">
  <span class="d-none d-sm-block">
    ...
    <form method="post" action="{{ route('update.admin.pass',$admin->id) }}" >
      @csrf
...
<script type="text/javascript">
  ...
  $('#imageUpload').change(function () {
    ...
    $('#reset').click(function() {
        ...
        $('#imagePreview').attr('src', "{{ (!empty($admin->photo)) ? asset('frontend/upload/admin_images/'.$admin->photo) : asset('frontend/upload/no_image.jpg') }}");
    });
...
document.getElementById('deactivateButton').addEventListener('click', function() {
    if (confirm("Are you sure you want to delete your account?\nOnce you delete your account, there is no going back. Please be certain.")) {
        ...
        window.location.href = '/admin/delete/admin/{{$admin->id}}';
    } else {
