...
@section('dashboard')
              ...
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Manage Experts /</span> Account</h4>
    ...
    <form method="post" action="{{ route('admin.edit.expert.info',$user->id) }}"  enctype="multipart/form-data" >
      @csrf
...
<img
  src="{{ (!empty($user->photo)) ? asset($user->photo) : asset('/frontend/upload/no_image.jpg') }}"
  alt="user-avatar"
...
<label for="imageUpload" tabindex="0">
  <span class="d-none d-sm-block">
        ...
        <select id="status" name="status" class="select2 form-select">
            ...
            <option value="inactive" {{ $user->status === 'inactive' ? 'selected' : '' }}>Inactive</option>
                        ...
                        <div class="mb-3 col-md-6">
                          ...
                          >{{$user->expert_short_info}}</textarea>
                        </div>
    ...
    <form method="post" action="{{ route('admin.edit.expert.pass',$user->id) }}" >
      @csrf
...
<script type="text/javascript">
  ...
  $('#imageUpload').change(function () {
    ...
    $('#reset').click(function() {
        ...
        $('#imagePreview').attr('src', "{{ (!empty($user->photo)) ? asset($user->photo) : asset('/frontend/upload/no_image.jpg') }}");
    });
...
document.getElementById('deactivateButton').addEventListener('click', function() {
    if (confirm("Are you sure you want to delete this account?\nOnce you delete your account, there is no going back. Please be certain.")) {
        ...
        window.location.href = '/admin/delete/expert/{{$user->id}}';
    } else {
