        ...
        class FirebaseConnection {
            constructor(db_ref, message_channel) {
                ...
                log('Talaka Chat.');
                log('Welcome, ' + username.value.trim());
    ...
    const audio = new Audio('/frontend/sounds/chat.wav');
    audio.loop = false;
        ...
        const fb_config = {
            apiKey: "AIzaSyB9Ge_HvihEDcGiltp6uYMS_0j6C6whZxI",
            authDomain: "talaka-c34af.firebaseapp.com",
            databaseURL: "https://talaka-c34af-default-rtdb.firebaseio.com",
            projectId: "talaka-c34af",
            storageBucket: "talaka-c34af.appspot.com",
            messagingSenderId: "883113647091"
        };
...
let onlineUsersElement = document.getElementById('online');
let onlineUsersCount = 0;

function updateOnlineUsersCount() {
    onlineUsersElement.textContent = 'Online: ' + onlineUsersCount;
}

// Function to increment online users count
function incrementOnlineUsersCount() {
    onlineUsersCount++;
    updateOnlineUsersCount();
}

// Function to decrement online users count
function decrementOnlineUsersCount() {
    onlineUsersCount--;
    updateOnlineUsersCount();
}
...
function init() {
    ...
    connection.on('disconnect', function(data) {
        ...
        if (peer_info) {
            ...
            decrementOnlineUsersCount(); // Decrement count when a user disconnects
        }
    });
}
        ...
        if (data) {
            ...
            users.forEach(function(data) {
                if (data.id !== id) {
                    ...
                    incrementOnlineUsersCount(); // Increment count when a new user joins
                }
            });
        }
    });
...
function fetchOnlineUsersCount() {
    // Replace this with your logic to fetch the count of online users from your database or server
    // For demonstration purposes, let's assume onlineUsersCount is already updated elsewhere
    // Initialize online users count to 0
    updateOnlineUsersCount();
}

// Update the count of online users every 5 seconds
